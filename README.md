...
[![Test Suite](https://img.shields.io/badge/Tests-144%20Passed-success.svg)](#)
[![License](https://img.shields.io/badge/License-MIT-blue.svg)](LICENSE)
[![DecodeLabs](https://img.shields.io/badge/DecodeLabs-Internship%20Submission-orange.svg)](#)
...
1. **Project 1 — 6-DOF Robotics Path Planner PRO**
2. **Project 2 — Automated Quality Inspection Workstation**
3. **Project 3 — Autonomous Mobile Robot (AMR) Navigation Engine**
...
```
Access the Industrial Inspection Portal at [http://localhost:5000](http://localhost:5000).
...
- **Dual Costmap Inflation Layers**: Global wall inflation ($r_{inf}=0.35\text{m}$) and local dynamic obstacle costmaps.
- **Dynamic Obstacle Avoidance**: Reflex Tanh deceleration curve $v(d) = v_{max} \cdot \tanh(\gamma \cdot (d - d_{min}))$ enforcing smooth slowdown and automatic path re-planning.
...
1. Open [http://localhost:5000/amr](http://localhost:5000/amr)
...
graph TD
    ...
    User <--> FlaskApp[Python Flask Web Server - Port 5000]

    subgraph "Project 1 - Robotics Engine"
        ReactUI --> Kinematics[KinematicsEngine - FK / IK]
        ReactUI --> Trajectory[TrajectoryEngine - Quintic]
        ReactUI --> Collision[CollisionEngine - 3D Volume]
        ReactUI --> ROS2Bridge[ROS 2 FollowJointTrajectory JSON]
    end

    subgraph "Project 2 - Computer Vision Workstation"
        FlaskApp --> CVEngine[15-Stage OpenCV Pipeline]
        CVEngine --> SyntheticData[Synthetic Dataset Generator]
        CVEngine --> DB[(SQLite3 Database)]
        CVEngine --> ReportGen[Report Generator - PDF/CSV/XLSX/JSON]
    end

    subgraph "Project 3 - AMR Navigation Engine"
        FlaskApp --> AMREngine[AMR Navigation Engine]
        AMREngine --> EKF[EKF Localization - Odom + IMU]
        AMREngine --> AStar[Custom A* Pathfinding - Manhattan]
        AMREngine --> Costmap[Global & Local Inflation Costmaps]
        AMREngine --> DynamicAvoidance[Dynamic Obstacle Avoidance & Tanh Deceleration]
    end

    subgraph "Industrial AI Suite"
        ReactUI --> IndustrialAI[src/industrial_ai/]
        IndustrialAI --> Anomaly[Anomaly Detector]
        IndustrialAI --> PredMaint[Predictive Maintenance Engine]
        IndustrialAI --> InspectAI[Inspection AI Classifier]
        IndustrialAI --> NLP[Prompt Security & NLP Assistant]
    end
```
...
Comprehensive project and API documentation is located in the [`docs/`](docs/) directory:
